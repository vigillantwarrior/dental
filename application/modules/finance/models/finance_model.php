...
class Finance_model extends CI_model {
    ...
    function getPaymentCategoryTypes() {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
    ...
    function getNewPatientsPaymentByDoctor($doctor) {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
        $this->db->select('*');
        $this->db->from('payment');
        $this->db->where('doctor', $doctor);
        $this->db->where('is_new_patient', 'Yes');
        $query = $this->db->get();
        return $query->result();
    }
    ...
    function getPaymentByDoctorDate($doctor, $date_from, $date_to) {
     ...
     function getNewPatientsPaymentByDoctorDate($doctor, $date_from, $date_to) {
        $this->db->where('hospital_id', $this->session->userdata('hospital_id'));
        $this->db->select('*');
        $this->db->from('payment');
        $this->db->where('is_new_patient', 'Yes');
     ...
     function getExecutivePatientsDoctor($doctor) {
       
        $this->db->select('*');
        $this->db->from('patient p');
        $this->db->join('payment pa', 'pa.patient = p.id');
        $this->db->where('pa.doctor', $doctor);
        $this->db->where('p.member_type', 'Executive');
        $query = $this->db->get();
        return $query->result();
    }

     function getFreeConsultationDoctor($doctor) {
       
        $this->db->select('*');
        $this->db->from('appointment');
        $this->db->where('doctor', $doctor);
        $this->db->where('type', 'free_consultation');
        $query = $this->db->get();
        return $query->result();
    }

    function getFreeConsultationDoctorDate($doctor, $date_from, $date_to) {
    ...
    function getExecutivePatientsDoctorDate($doctor, $date_from, $date_to) {
