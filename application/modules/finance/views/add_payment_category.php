                    ...
                    <div class="clearfix">
                        ...
                        <form role="form" action="finance/addPaymentCategory" class="clearfix" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control m-bot15 js-example-basic-single" name="type">    
                                      <?php foreach ($treatments as $treat) { ?>
                                        <option value="<?php echo $treat->type; ?>" <?php
                                        if (!empty($setval)) {
                                            if ($treat->type == set_value('type')) {
                                                echo 'selected';
                                            }
                                        }
                                        if (!empty($category->type)) {
                                            if ($category->type == $treat->type) {
                                                echo 'selected';
                                            }
                                        }
                                        ?> > <?php echo $treat->type; ?> </option>
                                            <?php } ?> 
                                </select>
                            </div>
