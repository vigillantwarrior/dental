...
class Pharmacy extends MX_Controller {
    ...
    public function addPaymentView() {
        ...
        $data['invoice'] = $this->pharmacy_model->getInvoicePaymentById($inv);
        $data['discount_type'] = $this->pharmacy_model->getDiscountType();
        $data['settings'] = $this->settings_model->getSettings();
        $data['medicines'] = $this->medicine_model->getMedicine();
        $this->load->view('home/dashboard', $data); // just the header file
        // $this->load->view('pharmacy/add_payment_view', $data);
        $this->load->view('pharmacy/add_payment_patient', $data);
        $this->load->view('home/footer'); // just the header file
    }
    ...
    public function addPayment() {
        ...
        foreach ($item_quantity_array as $key => $value) {
            ...
            $location_stock = $this->db->get_where('location_products', array('product_id' => $key, 'location_id' => '4'))->row();
            $current_stock = $location_stock->loc_quantity;
            $qty = $value;
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($id)) {
                ...
                foreach ($item_quantity_array as $key => $value) {
                    $previous_qty = $this->db->get_where('location_products', array('product_id' => $key, 'location_id' => '4'))->row()->loc_quantity;
                    $new_qty = $previous_qty - $value;
                    $this->db->where('product_id', $key);
                    $this->db->where('location_id', 4);
                    $this->db->update('location_products', array('loc_quantity' => $new_qty));
                }
    ...
    function editPayment() {
        if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Pharmacist'))) {
            ...
            $data['medicines'] = $this->medicine_model->getMedicine();
            $id = $this->input->get('id');
            $inv = $this->input->get('vol');
            $patient = $this->input->get('tron');
    ...
    function receipt() {
        ...
        $data['payment'] = $this->pharmacy_model->getPaymentById($id);
        $this->load->view('home/dashboard', $data); // just the header file
        $this->load->view('pharmacy/receipt', $data);
        $this->load->view('home/footer'); // just the footer fi
    }
